package syujaContigFilter::syujaContigFilterClient;
...
syujaContigFilter::syujaContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => syujaContigFilter::syujaContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
$contigset is a syujaContigFilter.contigset_id
$return is a syujaContigFilter.FilterContigResults
...
# Authentication: required
	...
	method => "syujaContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "syujaContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for syujaContigFilter::syujaContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "syujaContigFilter::syujaContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package syujaContigFilter::syujaContigFilterClient::RpcClient;
